package com.md1k.api.util;

/**
 * 常量类
 * @author 刘双桥
 *
 * 2017年6月2日
 */
public class Constants {

	public static final String SUCCESS = "success";
	
	public static final String YES = "1";
	
	public static final String NO = "0";
	
	public static final String PAGE_NO_FOUND = "404";
	
	public static final String Asterisk = "**";
	
	public static final String HTTP = "http";
	
	public static final String WWW = "www.";
	
	public static final String FRIEND_URL_WARING = "启用的友情链接数已经达到上限！";
	
	public static final String MANAGE_USERNAME = "sqliu";
	
	public static final String MANAGE_PASSWD = "lsq&&lsq520";


	public static final Integer WOMAN = 1;
	public static final Integer MAN = 2;
	public static final Integer SEX = 3;
	public static final Integer HEALTH = 4;
	public static final Integer COLD = 5;
	public static final Integer YOUNG = 6;
	public static final Integer HISTORY = 7;

}
